...
function initDemo(session, logger, env) {
  var filename = 'backend.js';

  var connectServer = (url) => {
    if(!url)
      return;
    ...
    $.get(url).done((data) => {
      ...
      if (session.fileNameCheck(filename)) {
        session.closeTab(session.getFileIdByName(filename));
      }       
      session.createNewFile(filename, json.code, 1, true);

      logger.info(`${filename} is loaded.`, true);
    });
  }
  ...
  var getAddress = () => {
    ...
    let address = `http://${$('#host-ip').val()}:${$('#host-port').val()}`;
    return address;
  }

  // register listeners
  $('#code-pull-button').on('click', (e) => {
    connectServer(getAddress());
    return true;
  });

  $('#connect-to-button').off('click');
  $('#connect-to-button').on('click', (e) => {
    var url = getAddress();
    logger.info(`Connect to: ${url}`);
    connectServer(getAddress(url));
    return true;
  });

  $('#code-push-button').on('click', (e) => {
    // console.log(env.editor.session.getValue());
    var url = getAddress();
    var code = env.editor.session.getValue();
  
    $.post(url + '/repl?', code).done(function(data) {
      // console.log(data);
      logger.info(`${filename} is sent.`, true);
    });
